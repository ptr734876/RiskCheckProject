...
def build_surroundings(lat: float, lon: float, radius: int, limit_per_kind: int = 3) -> dict:
    """
    Возвращает окружение точки в формате фронтенда.

    limit_per_kind ограничивает выдачу: в городе школ и дорог могут быть
    десятки, а в карточке нужны только ближайшие.
    """
